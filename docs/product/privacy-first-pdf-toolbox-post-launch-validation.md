...
| Split PDF | Simple, local-first, useful for forms and document cleanup | Requires page preview or clear page range input |
...
Start with `Split PDF` if user feedback is unclear. It is easier to validate than OCR or compression, stays local-first, and expands the current PDF workflow without introducing accounts, uploads, or third-party APIs.
